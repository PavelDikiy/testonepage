	...
	$.fn.TMForm=function(opt){
		...
		function TMForm(){
			...
			opt=$.extend({					
					...
					,successDopClass:'success_dop'
					,onceVerifiedClass:'once-verified'
					,mailHandlerURL:'/bat/MailHandler.php'					
					,successShowDelay:'7000'
					,successShowDelayDop:'3000'
					,stripHTML:true
				},opt)
...
/*					$.ajax({
						...
						,data:{
							...
							,phone2:getValue($('label.phone2 input'))
							,site:getValue($('label.site input'))
							,price:getValue($('label.price input'))
							,interval:getValue($('label.interval input'))
							,fax:getValue($('label.fax input'))
							,state:getValue($('label.state input'))
							,message:getValue($('label.message textarea'))
							,message2:getValue($('label.message2 textarea'))
							,owner_email:opt.ownerEmail||''
							,stripHTML:opt.stripHTML
						}
						,success: function(e){							
							form.addClass(opt.successClass)
							setTimeout(function(){form.addClass(opt.successDopClass);if (urll.indexOf("distrib") > 0){$("#form .btns .button").addClass('disemble');}},opt.successShowDelayDop)
							try{setTimeout(function(){
								if($("#callback").is(":checked")){$(".call_parent #form2 label[for='callback']").click();}
								if($("#pop_text").is(":checked")){$(".call_parent #form2 label[for='pop_text']").click();}
								},2000);}catch(er){}
							setTimeout(function(){
								form
								.removeClass(opt.successClass).removeClass(opt.successDopClass)
								.trigger('reset')
...
regula.custom({
	name:'JustLetters'
	,validator:function(){
		return /^[a-zA-Z'][a-zA-Z-' ]+[а-ьА-Ь']?$/.test(this.value)
	}
});
regula.custom({
	name:'JustNumbers'
...
regula.custom({
	name:'JustEmail'
	,validator:function(){
		return /^[-._a-z0-9]+@(?:[a-z0-9][-a-z0-9]+\.)+[a-z]{2,6}$/.test(this.value)
	}
});
